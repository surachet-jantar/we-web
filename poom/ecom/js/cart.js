/* เนื้อดี — Cart (localStorage) */
const cart = {
  getItems() { const d=localStorage.getItem('nd_cart'); return d?JSON.parse(d):[]; },
  saveItems(items) { localStorage.setItem('nd_cart',JSON.stringify(items)); this.updateBadge(); },
  addItem(productId, product, qty=1) {
    const items=this.getItems();
    const ex=items.find(i=>i.productId===productId);
    if(ex){ex.quantity+=qty;} else {
      items.push({productId, name_TH:product.Name_TH||'', cut:product.Cut||'',
  ...
  clear() { localStorage.removeItem('nd_cart'); this.updateBadge(); },
  ...
  buildOrder(user) {
    return {ID:'ORD_'+Date.now()+'_'+Math.random().toString(36).substr(2,6), UserID:user.ID,
      Items:JSON.stringify(this.getItems().map(i=>({productId:i.productId,name:i.name_TH,quantity:i.quantity,price:i.salePrice>0?i.salePrice:i.regularPrice}))),
